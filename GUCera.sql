...
BEGIN
Declare @var INT
select @var= id from users 
where email = @mail
set @id = @var
end

go 
create proc studentsucces
@sid int,
@cid int, 
@instid int,
@success bit OUTPUT
...
select @var = grade from StudentTakeCourse where sid = @sid AND cid= @cid AND insid =@instid
if (@var >2.0)
set @success=1
else
set @success =0
...
create proc Insname 
@id int,
@name1 varchar(10) OUTPUT,
@name2  varchar(10) OUTPUT
...
select @var1= firstName,@var2=lastName from Users
where id = @id
set @name1 = @var1
set @name2 = @var2
 ...
 go
create proc isEnroll
@id int,
@cid int,
@f bit output
as
begin
if( dbo.checkStudentEnrolledInCourse(@id,@cid)='1')
  set @f=1
else
  set @f=0
end
...
go
create proc isExistNum
@num varchar(15),
@var bit  output
as
begin
IF(EXISTS(SELECT * FROM CreditCard WHERE @num=number))
SET @var= '1'
ELSE
SET @var = '0'
end
-------------------------------
--procedure available courses modified:-
go

alter PROC availableCourses
AS
SELECT Course.name FROM Course where accepted='1'


go

create proc InsGrade
@instrId int,
@sid int ,
@cid int, 
@assignmentNumber int,
...
go
create proc checkInsT
@instrId int,
@cid int,
@s bit OUTPUT
as
if (exists(select *from Course where instructorId =@instrId AND id =@cid AND accepted =1))
set @s=1
else
set @s =0
...
go
create proc getCourseInst
@cid int
as
select it.insid,i.firstName,i.lastName from InstructorTeachCourse it inner join Course c on it.cid=c.id inner join Users i
on it.insid=i.id
where c.id=@cid
